...
const Home = () => {
    ...
    const [sections, setSections] = useState({
        ...
        cDrama: { title: 'C-Drama', data: [], loading: true, link: '/category/c-drama' },
    ...
    useEffect(() => {
        if (!category) {
            const fetchAll = async () => {
                ...
                fetchSection('cDrama', api.getCDrama);
    ...
    // Derived Latest Content (2026)
    const latestContent = useMemo(() => {
        if (category) return [];
        const allItems = [
            ...trending,
            ...(sections.kDrama?.data || []),
            ...(sections.cDrama?.data || []),
            ...(sections.shortTv?.data || []),
            ...(sections.anime?.data || []),
            ...(sections.westernTv?.data || []),
            ...(sections.indoDub?.data || [])
        ];

        const latest = allItems.filter(item => item.year === 2026 || item.year === '2026');
        // Deduplicate by ID
    ...
    const getApiCallByCategory = (cat) => {
        switch (cat) {
            ...
            case 'c-drama': return api.getCDrama;
